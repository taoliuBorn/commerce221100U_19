...
CPQ.config = {
    cpqBtnAddToCart: ".cpq-btn-addToCart",
    alertDangerMsg: ".alert-danger",
    alertInfoMsg: ".alert-info",
    ...
    registerOnClickHandlers: function () {
        ...
        $(CPQ.config.cpqBtnAddToCart).on("mousedown", function (e) {
            CPQ.config.addToCartClickTimestamp = e.timeStamp;
        });

        $(CPQ.config.cpqBtnAddToCart).on("mouseup", function (e) {
            if ((e.timeStamp - CPQ.config.addToCartClickTimestamp) < 2000) {
                CPQ.config.clickAddToCartButton();
            }
        });

        $(CPQ.config.cpqBtnAddToCart).on("keydown", function (e) {
            if (e.which === 13) {
                CPQ.config.clickAddToCartButton();
            }
        });
    ...
    ensureDangerMessagesAreVisible: function () {
        $(CPQ.config.alertDangerMsg).each(function () {
            if ($(this).parent().attr("class") === "global-alerts") {
                $(document).scrollTop($(CPQ.config.alertDangerMsg).offset().top - 10);
                // break the loop
                return false;
            }
            return true;
    ...
    ensureInfoMessagesAreVisible: function () {
        $(CPQ.config.alertInfoMsg).each(function () {
            if ($(this).parent().attr("class") === "global-alerts") {
                $(document).scrollTop($(CPQ.config.alertInfoMsg).offset().top - 10);
                //break the loop
                return false;
            }
            return true;
    ...
    ensureMessagesAreVisible: function () {
        if (($(CPQ.config.alertInfoMsg).length || $(CPQ.config.alertDangerMsg).length) && !conflicts) {
    ...
    getSerializedConfigForm: function (cpqAction, focusId, forceExpand,
        ...
        var data = $(CPQ.config.formNameId).serialize();
        $("#cpqAction").val("");
        $("#focusId").val("");
    ...
    registerAjax: function () {
        $(CPQ.config.formNameId).submit(function (e) {
            e.preventDefault();
        });
    ...
    clickAddToCartButton: function () {
        ...
        setTimeout(function addToCartClicked() {
            var form = $(CPQ.config.formNameId)[0];
    ...
    getDescriptionContext: function (messageFieldId, formElem) {
        var prevContent = formElem.attr(CPQ.config.ariaDescribedBy);
    ...
    generateAriaDescribedByForLabel: function (messageFieldId, csticId) {
        ...
        if (CPQ.config.isElementCsticLabel(formElem)) {
            formElem.attr(CPQ.config.ariaDescribedBy, newContent);
...
$(document).ready(function () {
    if ($("#cpq-dynamic-config-content").length > 0) {
        ...
        CPQ.core.formNameId = CPQ.config.formNameId;
        CPQ.config.bindAll();
    }
});
