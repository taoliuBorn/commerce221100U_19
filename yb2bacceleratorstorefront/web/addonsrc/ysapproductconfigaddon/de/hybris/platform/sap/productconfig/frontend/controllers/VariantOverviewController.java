...
public class VariantOverviewController extends AbstractConfigurationOverviewController
{
	...
	/**
	 * Renders the product configuration overview page, using an entry number and a product configuration variant as
	 * input.
	 *
	 * @param productCodeEncoded
	 *           product code of the variant
	...
	protected ConfigurationOverviewData prepareOverviewData(final String productCodeEncoded)
	{
		...
		{
			LOGGER.debug(CONFIG_GET_RECEIVED_FOR + productCode + CURRENT_SESSION + getSessionId() + CLOSING_QUOTE);
		}
	...
	public void addVariantToCartCleanUp(@PathVariable("productCode") final String productCodeEncoded)
	{
		...
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug("AddVariantToCart GET received for '" + productCode + CURRENT_SESSION + getSessionId() + CLOSING_QUOTE);
		}
			...
			final HttpServletRequest request) throws CMSItemNotFoundException
	{
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug(CONFIG_POST_RECEIVED_FOR + productCode + CURRENT_SESSION + getSessionId() + CLOSING_QUOTE);
		}
